/*
 ...
 Modified for Particle by Chip McClelland - (chip at SeeInsights dot com)
 ...
 This library was written for use with any Arduino board. Chip McClelland modified this library to work with Particle.

 Note from Sridhar - The DS1337 library is suited for Mosquino, and you have to make some modifications to make it work elsewhere. 
 I saw that the DS1337 and DS1339 both have similar functionality and data format (the DS1339 has an extra trickle charger in 
 case you want to use a super cap instead of a coin cell battery. I've modified the library to allow for the configuration settings
...
DS1337::DS1337() : DSRTCLib(2)
{
}

DS1337::DS1337(int int_pin) : DSRTCLib(int_pin)
{
}
